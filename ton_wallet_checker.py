...
def has_exchange_interaction(transactions_compact: list, known_addresses: set[str]) -> bool:
    """
    Приймає compact-рядки виду:
      {'source': {...}|str, 'destination': {...}|str, 'value_TON': float, ...}
    Повертає True, якщо source або destination належать відомим біржам.
    """
    for tx in transactions_compact:
        src = tx.get("source")
        dst = tx.get("destination")
        src_addr = (src.get("address") if isinstance(src, dict) else src) or ""
        dst_addr = (dst.get("address") if isinstance(dst, dict) else dst) or ""
        if src_addr in known_addresses or dst_addr in known_addresses:
            return True
    return False

# ---- Safety assessment function ----
def assess_safety(rows):
    score = 0

    if has_exchange_interaction(rows, known_exchange_addresses):
        score += 2

    tiny_transfers = [tx for tx in rows if tx["value_TON"] < 0.001]
